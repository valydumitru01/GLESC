 ...
 * The circular dependency arrives due to the fact that MatrixAlgorithms uses vector operations and
 * VectorAlgorithms uses matrix operations. For example, the collinearity of two vectors is checked
 * making use of a matrix. Matrix operations could be implemented inside VectorAlgorithms, but this
 * would be a bad design choice. The solution is to create a new class VectorBasicAlgorithms that
 * contains only the basic vector operations.
 *
...
namespace GLESC::Math {
    ...
    public:
        ...
        static inline void normalize(const VectorData<Type, N> &vec, VectorData<Type, N> &result) {
            Type length = VectorBasicAlgorithms::length(vec);
        ...
        project(const VectorData<Type, N> &vec, const VectorData<Type, N> &onto, VectorData<Type, N> &result) {
            ...
            VectorBasicAlgorithms::vectorScalarMul(onto, scalarProjection, result);
        }
