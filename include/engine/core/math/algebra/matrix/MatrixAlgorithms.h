...
namespace GLESC::Math {
    ...
    public:
        ...
        static inline MatrixData<Type, N2, M2> resizeMatrix(const MatrixData<Type, N1, M1> &matrix) {
        ...
        static inline void swapRows(MatrixData<Type, N, M> &matrix, size_t row1, size_t row2) {
        ...
        template<typename Type, size_t N, size_t M>
        static inline void setMatrxZero(MatrixData<Type, N, M> &matrixObjective) {
        ...
        template<typename Type, size_t N, size_t M>
        static inline void setMatrixDiagonal(MatrixData<Type, N, M> &matrixObjective, const Type &value) {
            for (size_t i = 0; i < N; ++i)
                for (size_t j = 0; j < M; ++j)
                    matrixObjective[i][j] = (i == j) ? value : 0;
        }
        
        template<typename Type, size_t N, size_t M>
        static inline void setMatrix(MatrixData<Type, N, M> &matrixObjective, const MatrixData<Type, N, M> &values) {
            copyMatrix(matrixObjective, values);
        }
        
        template<typename Type, size_t N, size_t M>
        static inline void setMatrix(MatrixData<Type, N, M> &matrixObjective, const Type (&values)[N][M]) {
            for (size_t i = 0; i < N; ++i)
                for (size_t j = 0; j < M; ++j)
        ...
        template<typename Type, size_t N, size_t M>
        static inline void
        copyMatrix(MatrixData<Type, N, M> &matrixObjective, const MatrixData<Type, N, M> &matrixToCopy) {
         ...
         */
        template<typename Type, size_t N, size_t M>
        static inline void moveMatrix(MatrixData<Type, N, M> &matrixObjective, MatrixData<Type, N, M> &&matrixToMove) {
         ...
         */
        template<typename Type, size_t N, size_t M>
        static inline void matrixAdd(const MatrixData<Type, N, M> &matrix1,
         ...
         */
        template<typename Type, size_t N, size_t M>
        static inline void matrixHadamardMul(const MatrixData<Type, N, M> &matrix1,
                                             const MatrixData<Type, N, M> &matrix2,
                                             MatrixData<Type, N, M> &result) {
            for (size_t i = 0; i < N; ++i)
                for (size_t j = 0; j < M; ++j)
         ...
         */
        template<typename Type, size_t N, size_t M>
        static inline void matrixSub(const MatrixData<Type, N, M> &matrix1,
                                     const MatrixData<Type, N, M> &matrix2,
                                     MatrixData<Type, N, M> &result) {
            for (size_t i = 0; i < N; ++i)
                for (size_t j = 0; j < M; ++j)
         ...
         */
        template<typename Type, size_t N, size_t M>
        static inline void matrixNegate(const MatrixData<Type, N, M> &matrix, MatrixData<Type, N, M> &result) {
            for (size_t i = 0; i < N; ++i)
                for (size_t j = 0; j < M; ++j)
         ...
         */
        template<typename Type, size_t N, size_t M>
        static inline void matrixHadamardDiv(const MatrixData<Type, N, M> &matrix1,
                                             const MatrixData<Type, N, M> &matrix2,
                                             MatrixData<Type, N, M> &result) {
            for (size_t i = 0; i < N; ++i)
                for (size_t j = 0; j < M; ++j) {
        ...
        matrixScalarDiv(const MatrixData<Type, N, M> &matrix, const Type &scalar, MatrixData<Type, N, M> &result) {
            D_ASSERT_TRUE(scalar != 0, "Division by zero is not allowed.");
        ...
        static inline void matrixMul(const MatrixData<Type, N, M> &matrix1,
         ...
         */
        template<typename Type, size_t N, size_t M, size_t X>
        static inline void matrixDiv(const MatrixData<Type, N, M> &matrix1,
                                     const MatrixData<Type, M, X> &matrix2,
                                     MatrixData<Type, N, X> &result) {
            MatrixData<Type, M, X> inverse;
            matrixInverse(matrix2, inverse);
            matrixMul(matrix1, inverse, result);
        }
         ...
         */
        template<typename Type, size_t N, size_t M>
        static inline void transpose(const MatrixData<Type, N, M> &matrix, MatrixData<Type, M, N> &result) {
            for (size_t i = 0; i < N; ++i)
                for (size_t j = 0; j < M; ++j)
        ...
        inverse2x2(const MatrixData<Type, N, N> &matrix, MatrixData<Type, N, N> &result) {
            ...
            D_ASSERT_TRUE(determinant != 0, "Matrix is not invertible.");
            D_ASSERT_TRUE(&matrix != &result, "Cannot invert matrix in place.");
        ...
        inverse4x4(const MatrixData<Type, N, N> &matrix, MatrixData<Type, N, N> &result) {
            ...
            result[0][0] = determinant * (matrix[1][1] * A2323 -  matrix[1][2] * A1323 + matrix[1][3] * A1223);
            result[0][1] = determinant * -(matrix[0][1] * A2323 - matrix[0][2] * A1323 + matrix[0][3] * A1223);
            result[0][2] = determinant * (matrix[0][1] * A2313 -  matrix[0][2] * A1313 + matrix[0][3] * A1213);
            result[0][3] = determinant * -(matrix[0][1] * A2312 - matrix[0][2] * A1312 + matrix[0][3] * A1212);
            result[1][0] = determinant * -(matrix[1][0] * A2323 - matrix[1][2] * A0323 + matrix[1][3] * A0223);
            result[1][1] = determinant * (matrix[0][0] * A2323 -  matrix[0][2] * A0323 + matrix[0][3] * A0223);
            result[1][2] = determinant * -(matrix[0][0] * A2313 - matrix[0][2] * A0313 + matrix[0][3] * A0213);
            result[1][3] = determinant * (matrix[0][0] * A2312 -  matrix[0][2] * A0312 + matrix[0][3] * A0212);
            result[2][0] = determinant * (matrix[1][0] * A1323 -  matrix[1][1] * A0323 + matrix[1][3] * A0123);
            result[2][1] = determinant * -(matrix[0][0] * A1323 - matrix[0][1] * A0323 + matrix[0][3] * A0123);
            result[2][2] = determinant * (matrix[0][0] * A1313 -  matrix[0][1] * A0313 + matrix[0][3] * A0113);
            result[2][3] = determinant * -(matrix[0][0] * A1312 - matrix[0][1] * A0312 + matrix[0][3] * A0112);
            result[3][0] = determinant * -(matrix[1][0] * A1223 - matrix[1][1] * A0223 + matrix[1][2] * A0123);
            result[3][1] = determinant * (matrix[0][0] * A1223 -  matrix[0][1] * A0223 + matrix[0][2] * A0123);
            result[3][2] = determinant * -(matrix[0][0] * A1213 - matrix[0][1] * A0213 + matrix[0][2] * A0113);
            result[3][3] = determinant * (matrix[0][0] * A1212 -  matrix[0][1] * A0212 + matrix[0][2] * A0112);
        }
        ...
        template<typename Type, size_t N>
        static inline void
        scale(const MatrixData<Type, N, N> &matrix, const VectorData<Type, N - 1> &scale,
              MatrixData<Type, N, N> &result) {
            D_ASSERT_TRUE(N == 3 || N == 4, "Scaling only makes sense for 3x3 and 4x4 matrices.");
            setMatrix(result, matrix);
            for (size_t i = 0; i < N - 1; ++i) {
                result[i][i] *= scale[i];
            }
        ...
        static inline void determinant2x2(const MatrixData<Type, 2, 2> &matrix, Type &result) {
        ...
        template<typename Type, size_t N, size_t M>
        static inline GaussianEliminationData<Type, N, M>
    ...
    private:
        ...
        template<typename Type, size_t N, size_t M>
        static inline GaussianEliminationTrackedResult<Type, N, M>
        gaussianEliminationAlgorithm(const MatrixData<Type, N, M> &inputMatrix) {
            ...
            for (size_t i = 0; i < N; ++i) {
                for (size_t j = 0; j < M; ++j) {
                    trackedResult.resultMatrix[i][j] = inputMatrix[i][j];
                }
            }
